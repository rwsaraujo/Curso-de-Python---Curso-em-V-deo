"""
NOME/NAME: Raphael Araújo
DATA/DATE: 01/11/2019
[PT]
> Faça um programa que leia nome e média de um aluno, guardando também a situação em um dicionário. No final, mostre o
conteúdo da estrutura na tela.
[EN]
> Make a program that reads a student's name and average, while also keeping the situation in a dictionary. At the end,
show the contents of the structure on the screen.
"""
#Gustavo Guanabára
aluno = dict()
aluno['nome'] = str(input('Nome: '))
aluno['média'] = float(input(f'Média de {aluno["nome"]}: '))
if aluno['média'] >= 7:
  aluno['situação'] = 'Aprovado'
elif 5 <= aluno['média'] < 7:
  aluno['situação'] = 'Recuperação'
else:
  aluno['situação'] = 'Reprovado'
print('-=' * 30)
for k, v in aluno.items():
  print(f'  - {k} é igual a {v}')